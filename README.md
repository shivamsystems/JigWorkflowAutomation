...
┌─────────────────────────────────────────────────────────────┐│                  JIG WORKFLOW AUTOMATION                     │├──────────────────────┬──────────────────────────────────────┤│                      │                                      ││   SOLIDWORKS FLOW    │       3D PRINTING FLOW               ││                      │                                      ││  SW Opens ──────────►│  BambuStudio Closes ──────────────► ││       │              │         │                            ││  ┌────▼─────┐        │   ┌─────▼──────────┐                ││  │ Project  │        │   │ Print &        │                ││  │ Details  │        │   │ Filament Log   │                ││  └────┬─────┘        │   └─────┬──────────┘                ││       │              │         │                            ││  SW Closes ────────► │         │                            ││       │              │         │                            ││  ┌────▼─────────┐    │         │                            ││  │ Session End  │    │         │                            ││  │ Summary      │    │         │                            ││  └────┬─────────┘    │         │                            │├───────┴──────────────┴─────────┴────────────────────────────┤│                  5:15 PM SCHEDULER                           ││  • Triggers daily handover log                               ││  • Smart skip logic (Reminds every 15m, Deadline at 6PM)    ││  • Next-day startup check for missed logs                    │├──────────────────────────────────────────────────────────────┤│                  EXCEL LOGGER (Thread-Safe)                  ││  • SolidWorks Projects Sheet                                ││  • Print & Filament Log Sheet                               ││  • Daily Handovers Sheet                                    │└──────────────────────────────────────────────────────────────┘
🚀 Installation & Setup
Prerequisites
Windows 10/11
...
git clone https://github.com/YOUR_USERNAME/JigWorkflowAutomation.git
...
Note: settings.json, Jig_Workflow_Log.xlsx, and jig_workflow.log are auto-generated on first run and ignored by Git.

🛠️ Built With
Python - Core logic
Tkinter - Dashboard & Popups
OpenPyXL - Excel data logging
